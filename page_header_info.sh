...
dsk_lsn=`dd status=none bs=8192 count=1 if=$relpath skip=$blk | od -A n -t x8 -N 8`
...
printf "%-8s %1s %-16s %1s %-11s %1s %-12s %1s %-12s %1s %-12s %1s %-12s %1s %-10s \n" 'disk' '|' $dsk_lsn '|' $dsk_checksum '|' $dsk_flags '|' $dsk_lower '|' $dsk_upper '|' $dsk_special '|' $dsk_prune
fi

if [ "$inspect" = "mem" -o "$inspect" = "both" ] ; then
from_mem=`psql -tA -c "SELECT * FROM page_header(get_raw_page('$tb', $blk))"`
